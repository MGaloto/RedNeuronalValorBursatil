...
datos = cbind(prediccion$net.result, test.sc)

names(datos)[1] = 'prediccion'

error_abs = RMSE(datos$precio, datos$prediccion, na.rm = TRUE)
error_por = error_abs / datos[datos$fecha == max(na.omit(datos)$fecha),]$precio


ggplot() + geom_line(data = subset(datos, datos$fecha >= '2019-07-15'), aes(x = fecha, y = precio), color = "blue") +
  geom_line(data = subset(datos, datos$fecha >= '2019-07-15'), aes(x = fecha, y = prediccion), color = "red") 



# probaremos el modelo random forest
...
mod_rf = randomForest(precio ~ año + mes + dia, data = AAPL_P[train,],
                      type = "regresion", ntree = 100)
# por defecto hace 500 pero le ponemos 100 y que sea regresion

prediccion_rf = predict(mod_rf, test)

datos_rf = cbind(prediccion_rf, test)

error_abs_rf = RMSE(datos_rf$precio, datos_rf$prediccion_rf, na.rm = T)

error_por_rf = error_abs_rf / datos_rf[datos_rf$fecha_dup == max(na.omit(datos_rf)$fecha_dup),]$precio



ggplot() + geom_line(data = subset(datos_rf, datos_rf$fecha_dup >= '2018-07-15'), aes(x = fecha_dup, y = precio), color = "blue") +
  geom_line(data = subset(datos_rf, datos_rf$fecha_dup >= '2018-07-15'), aes(x = fecha_dup, y = prediccion_rf), color = "red") 
